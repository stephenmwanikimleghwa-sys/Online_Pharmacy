...
# Resolve the CSV file path relative to the Django project root
_MIGRATIONS_DIR = Path(__file__).resolve().parent.parent.parent
# The CSV is in the root of the repo, which is one level above backend
_CSV_FILE = _MIGRATIONS_DIR.parent / "product_branch_inventory.csv"

def import_legacy_prices_and_categories(apps, schema_editor):
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('products', '0017_fix_categories_and_prices'),
    ]

    operations = [
        migrations.RunPython(import_legacy_prices_and_categories, reverse_code=migrations.RunPython.noop),
    ]
