      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-12 flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <h1 className="text-4xl font-display font-bold tracking-tight" style={{color:'var(--text-primary)'}}>Dispensing <span className="text-primary">Terminal</span></h1>
            </div>
            <p className="text-lg font-medium" style={{color:'var(--text-secondary)'}}>Verify stock availability and authorize medication release to patient.</p>
          </div>
          <button
            onClick={() => navigate('/pharmacist/dashboard')}
        ...
        <div className="glass-card rounded-[2.5rem] p-10 border border-[var(--border-primary)] shadow-premium mb-10 relative overflow-hidden">
          ...
          <div className="flex items-center gap-3 mb-8">
            ...
            <h2 className="text-xl font-display font-bold" style={{color:'var(--text-primary)'}}>Patient Specification</h2>
          </div>
          ...
          {prescription.verified_by && (
            <div className="mt-10 pt-8 border-t flex items-center gap-4" style={{borderColor:'var(--border-primary)', color:'var(--text-secondary)'}}>
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" /></svg>
              <p className="text-xs font-medium">
                Authorized by <span className="font-bold" style={{color:'var(--text-primary)'}}>{prescription.verified_by?.full_name}</span>
                {prescription.verified_at && (
                  <> on <span className="font-bold" style={{color:'var(--text-primary)'}}>{new Date(prescription.verified_at).toLocaleDateString()}</span></>
                )}
              </p>
            </div>
          )}
        ...
        <div className="glass-card rounded-[2.5rem] p-10 border border-[var(--border-primary)] shadow-premium mb-10">
          <div className="flex items-center gap-3 mb-10">
            ...
            <h2 className="text-xl font-display font-bold" style={{color:'var(--text-primary)'}}>Release Protocol</h2>
          </div>
                ...
                <div key={index} className="data-cell p-8 rounded-[2rem] transition-all hover:shadow-premium group">
                  <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8">
                    ...
                    <div className="flex flex-col items-end gap-4 min-w-[200px]">
                      ...
                      {isAvailable && (
                        <div className="flex items-center gap-3 p-2 rounded-2xl border shadow-sm focus-within:ring-4 focus-within:ring-[var(--color-primary)]/10 transition-all" style={{background:'var(--bg-field)', borderColor:'var(--border-primary)'}}>
                          <input
                            type="number"
                            min="0"
        ...
        <div className="glass-card rounded-[2.5rem] p-8 border border-[var(--border-primary)] shadow-premium">
          <div className="flex flex-col md:flex-row justify-between items-center gap-8 text-center md:text-left">
            <div>
              ...
              <p className="text-sm font-medium" style={{color:'var(--text-secondary)'}}>Verify all release quantities before final ledger commitment.</p>
            </div>
