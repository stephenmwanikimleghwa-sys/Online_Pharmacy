    ...
    <div className="space-y-6 animate-fade-in max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <PageHeader
        ...
        actions={
          <>
            ...
            >
              <PlusIcon className="w-4 h-4" /> New supplier
            </button>
          </>
        }
      />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="glass-card rounded-xl p-4 border" style={{ borderColor: 'var(--border-primary)' }}>
          ...
          <p className="text-2xl font-display font-bold" style={{ color: 'var(--text-primary)' }}>{suppliers.length}</p>
        </div>
      ...
      {/* Toolbar */}
      <div className="glass-card rounded-xl border p-4 flex flex-col md:flex-row gap-4" style={{ borderColor: 'var(--border-primary)' }}>
        <div className="relative flex-1">
          {!search && <MagnifyingGlassIcon className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4" style={{ color: 'var(--text-secondary)' }} />}
          <input
            type="text"
            placeholder="Search suppliers…"
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            className={`form-input w-full ${search ? 'pl-3' : 'pl-9'}`}
          />
        </div>
        <div className="relative w-full md:w-64">
          <FunnelIcon className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4" style={{ color: 'var(--text-secondary)' }} />
          <select
            value={filterDebt}
          ...
          {filteredSuppliers.map(supplier => {
            ...
            return (
              <button
                ...
                className="text-left group relative glass-card rounded-xl p-3 border  shadow-sm hover:shadow-premium hover:border-primary/20 transition-all duration-300"
              >
      ...
      {isFormModalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm overflow-y-auto h-full w-full z-[60] flex items-center justify-center animate-fade-in p-4">
            <div className="relative bg-white rounded-xl max-w-md w-full mx-4 animate-scale-up">
                <div className="p-8">
