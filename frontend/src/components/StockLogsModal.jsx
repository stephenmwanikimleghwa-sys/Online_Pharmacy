      ...
      <div className="modal-card w-full max-w-5xl max-h-[90vh] overflow-hidden flex flex-col animate-scale-up">
        ...
        <div className="px-10 py-8 flex justify-between items-center border-b" style={{borderColor:'var(--border-primary)', background:'var(--bg-field)'}}>
          <div>
            ...
            <p className="text-xs font-bold mt-1" style={{color:'var(--text-secondary)'}}>See when stock went up or down</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-10" style={{background:'var(--bg-primary)'}}>
          {loading ? (
            <div className="flex flex-col justify-center items-center h-64 gap-4 opacity-40">
              ...
              <p className="text-xs font-bold animate-pulse" style={{color:'var(--text-secondary)'}}>Loading...</p>
            </div>
          ) : error ? (
            <div className="alert-error rounded-xl p-8">
            ...
            <div className="data-cell rounded-xl p-20 text-center flex flex-col items-center opacity-60">
              <div className="w-20 h-20 rounded-xl flex items-center justify-center mb-6 shadow-inner" style={{background:'var(--bg-card)'}}>
            ...
            <div className="overflow-hidden rounded-xl border shadow-sm" style={{borderColor:'var(--border-primary)'}}>
              <table className="min-w-full divide-y" style={{borderColor:'var(--border-primary)'}}>
                <thead className="table-header-row">
                  <tr>
                    {['Date & time', 'Type', 'Change', 'Reason', 'By'].map((h) => (
                      <th key={h} className="px-8 py-5 text-left text-xs font-bold" style={{color:'var(--text-secondary)'}}>{h}</th>
                    ))}
                  </tr>
                </thead>
                  ...
                  {logs.map((log) => (
                    <tr key={log.id} className="transition-colors" style={{}} onMouseEnter={e => e.currentTarget.style.background='var(--bg-field)'} onMouseLeave={e => e.currentTarget.style.background=''}>
                      ...
                      <td className="px-8 py-6">
                        <span className={`px-3 py-1 rounded-xl text-xs font-bold border ${log.change_type === 'restock' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' :
                      ...
                      <td className="px-8 py-6">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="text-xs font-semibold tracking-tighter" style={{color:'var(--border-primary)'}}>
                      ...
                      <td className="px-8 py-6">
                        <div className="flex items-center gap-2">
                          <div className="w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold" style={{background:'var(--brand-mist)', color:'var(--text-secondary)'}}>
                            {log.logged_by_username?.[0]?.toUpperCase() || 'S'}
                          </div>
                          <span className="text-xs font-semibold tracking-tight" style={{color:'var(--text-primary)'}}>{log.logged_by_username || 'System'}</span>
                        </div>
                      </td>
                    </tr>
                  ))}
        ...
        <div className="px-10 py-6 border-t flex justify-end" style={{borderColor:'var(--border-primary)', background:'var(--bg-field)'}}>
          <button
            onClick={onClose}
            className="btn-primary px-8 py-3 rounded-2xl font-bold text-xs transition-all active:scale-95"
          >
            Close
          </button>
        </div>
